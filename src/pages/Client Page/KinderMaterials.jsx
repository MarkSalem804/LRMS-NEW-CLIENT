  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      ...
      try {
        ...
        if (response.success) {
          // Only Kindergarten
          const kinder = response.data.filter((m) =>
            ["Kindergarten"].includes(m.gradeLevelName)
          );
          setMaterials(kinder);
          setFilteredMaterials(kinder);
          // Extract unique filter options
          setGradeLevels([
            ...new Set(kinder.map((m) => m.gradeLevelName).filter(Boolean)),
          ]);
          setLearningAreas([
            ...new Set(kinder.map((m) => m.learningAreaName).filter(Boolean)),
          ]);
          setComponents([
    ...
    try {
      ...
      if (response.success) {
        ...
        if (materialsResponse.success) {
          const kinder = materialsResponse.data.filter((m) =>
            ["Kindergarten"].includes(m.gradeLevelName)
          );
          setMaterials(kinder);
          setFilteredMaterials(kinder);
        ...
        <main className="flex-grow container mx-auto px-4 py-16">
          <div className="flex items-center justify-between mb-10 gap-4">
            <button
              ...
              className="flex items-center bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg shadow transition-colors font-medium"
            >
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-4 mb-8 bg-black border border-black rounded-lg shadow p-4">
            <input
              ...
              className="w-full md:w-1/3 px-4 py-2 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-400"
            />
            <select
              value={gradeLevel}
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-300 bg-white/80">
                  <div style={{ maxHeight: 480, overflowY: "auto" }}>
                    <table className="min-w-full text-sm">
                      <thead className="bg-gradient-to-r from-pink-500 to-pink-400">
                        <tr>
                                ...
                                <div className="flex items-center justify-center gap-2">
                                  <button
                                    ...
                                    className={`p-2 rounded-full shadow ${
                                      hasViewableFile(material)
                                        ? "bg-white hover:bg-pink-100 text-pink-700 cursor-pointer"
                  ...
                  <div className="flex justify-between items-center p-4">
                    <button
                      ...
                      disabled={page === 1}
                      className="px-3 py-1 rounded bg-pink-500 text-white disabled:bg-gray-300"
                    >
                      Previous
                    </button>
                    <span>
                ...
                >
                  {filteredMaterials.map((material) =>
                    view === "card" ? (
                      <div
                        key={material.id}
                        className="bg-gradient-to-br from-pink-100 to-pink-300 rounded-lg shadow-md p-2 sm:p-2.5 md:p-3 transition-transform duration-200 hover:-translate-y-1 hover:shadow-lg"
                      >
                        <div className="flex justify-between items-start mb-1.5 gap-1.5">
                          <h3 className="text-xs sm:text-sm md:text-base font-semibold text-pink-700 flex-1 line-clamp-2">
                            {material.title}
                          </h3>
                      ...
                      <div
                        key={material.id}
                        className="flex flex-col sm:flex-row items-start bg-gradient-to-br from-pink-100 to-pink-300 rounded-lg shadow-md p-2 sm:p-2.5 md:p-3 gap-2 sm:gap-3 hover:shadow-lg"
                      >
...
export default KinderMaterials;
