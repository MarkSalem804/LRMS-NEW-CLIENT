  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      ...
      try {
        ...
        if (response.success) {
          // Only Elementary
          const elem = response.data.filter((m) =>
            [
              "Grade 1",
              "Grade 2",
              "Grade 3",
          ...
          setGradeLevels([
            ...new Set(elem.map((m) => m.gradeLevelName).filter(Boolean)),
          ]);
          setLearningAreas([
            ...new Set(elem.map((m) => m.learningAreaName).filter(Boolean)),
          ]);
          setComponents([
            ...new Set(elem.map((m) => m.componentName).filter(Boolean)),
          ]);
          setResourceTypes([
            ...new Set(elem.map((m) => m.typeName).filter(Boolean)),
          ]);
        }
      } finally {
        setIsLoading(false);
